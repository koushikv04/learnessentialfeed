...
final class FeedLocalisationTests:XCTestCase {
    func test_localisedString_hasKeyandValuesForAllLocalizations() {
        let table = "Feed"
        let bundle = Bundle(for: FeedPresenter.self)
        let localisationBundles = allLocalizationBundles(in:bundle)
        let localisationStringKeys = allLocalizedStringKeys(in:localisationBundles, table:table)
        localisationBundles.forEach { bundle,localization in
            localisationStringKeys.forEach { key in
